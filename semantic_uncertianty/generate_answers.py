...
def main(args):
    ...
    for dataset_split in ['train', 'validation']:
        ...
        for index in tqdm(indices):
            ...
            total_num_generated_tokens_instance = 0
            total_num_reuse_tokens_instance = 0
            for i in range(num_generations):
                # Temperature for first generation is always `0.1`.
                temperature = 0.1 if i == 0 else args.temperature
                tic = time.time()
                if not args.enable_reuse:
                    ...
                    predicted_answer, token_log_likelihoods, (embedding, emb_last_before_gen, emb_before_eos), cache, (num_generated_tokens, num_reuse_tokens) = \
                    model.predict_with_cache(local_prompt, temperature, topk=args.topk, topp=args.topp, return_latent=True, cache=cache) 
                toc = time.time()
                total_time_instance += toc - tic
                if args.enable_reuse:
                    total_num_generated_tokens_instance += num_generated_tokens
                    total_num_reuse_tokens_instance += num_reuse_tokens
                ...
                if i == 0:
                    ...
                    # store the embedding if compute_p_ik is enabled
            ...
            logging.info(f"Time taken for generation: {total_time_instance} seconds")
            if args.enable_reuse:
                logging.info(f"Number of generated tokens: {total_num_generated_tokens_instance}")
                logging.info(f"Number of reuse tokens: {total_num_reuse_tokens_instance}")
                logging.info(f"Number of reuse ratio: {total_num_reuse_tokens_instance / total_num_generated_tokens_instance}")
            total_time += total_time_instance
            if args.enable_reuse:
                total_num_generated_tokens += total_num_generated_tokens_instance
            total_num_reuse_tokens += total_num_reuse_tokens_instance
    ...
    del model
    logging.info(f'Total time taken for the run: {total_time} seconds')
    if args.enable_reuse:
        logging.info(f'Total number of generated tokens: {total_num_generated_tokens}')
        logging.info(f'Total number of reuse tokens: {total_num_reuse_tokens}')
        logging.info(f'Total number of reuse ratio: {total_num_reuse_tokens / total_num_generated_tokens}')
