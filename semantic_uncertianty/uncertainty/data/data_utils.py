...
def load_ds(dataset_name, seed, add_options=None):
    ...
    if dataset_name == "squad":
        ...
        # Currently only support validation dataset
        validation_dataset = datasets.load_dataset("bigbio/sciq", "sciq_bigbio_qa")['validation']
        ...
        validation_dataset = datasets.load_dataset("pminervini/HaluEval", "qa")['data']
        md5hash = lambda s: str(int(hashlib.md5(s.encode('utf-8')).hexdigest(), 16))

        reformat = lambda x: {
            'question': x['question'],
            'answers': {'text': x['right_answer']},
            'context': '',
            'id': md5hash(str(x['question'])),
        }
