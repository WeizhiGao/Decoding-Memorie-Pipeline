...
BRIEF_PROMPTS = {
    ...
    'last': 'In a single, brief, and complete sentence, provide the exact answer to the following question at the end of the sentence. \n'}
...
def get_parser(stages=['generate', 'compute']):
    ...
    if 'generate' in stages:
        ...
        parser.add_argument(
            ...
            choices=['trivia_qa', 'squad', 'bioasq', 'nq', 'svamp', 'sciq', 'halueval'],
            help="Dataset to use")
        parser.add_argument(
        ...
        parser.add_argument(
            "--enable_reuse", default=False,
            action=argparse.BooleanOptionalAction,
            help='Enable reuse techniques')
        parser.add_argument(
            "--reuse_mode", default='hard', type=str,
            choices=['soft', 'hard'],
            help="the mode of reusing")
        parser.add_argument(
            "--reuse_prob", default=0.85, type=float,
            help="the probability of reusing")
        parser.add_argument(
            "--topk", default=50, type=int,
            help="the topk of topk-topp filtering")
        parser.add_argument(
            "--topp", default=0.9, type=float,
            help="the topp of topk-topp filtering")
        parser.add_argument(
            "--short_answer_length", default=10, type=int,
            help="the length of short answer")
        parser.add_argument(
            "--enable_reweight", default=False,
            action=argparse.BooleanOptionalAction,
            help='Enable reweighting')
        parser.add_argument(
            "--threshold", default=0.9, type=float,
            help="the threshold of reweighting")
        parser.add_argument(
            "--reweight_scaling", default=1.1, type=float,
            help="the scaling of reweighting")
    ...
    if 'compute' in stages:
        ...
        parser.add_argument('--local_reload_path', type=str, default=None)
    return parser
...
def model_based_metric(predicted_answer, example, model):
    ...
    if predicted_answer is None:
        logging.warning('Sensitive question. Return no.')
        return 0.0
...
def init_model(args):
    ...
    if 'llama' in mn.lower() or 'falcon' in mn.lower() or 'mistral' in mn.lower() or 'phi' in mn.lower():
        model = HuggingfaceModel(
            ...
            enable_reweight=args.enable_reweight,
            threshold=args.threshold,
            reweight_scaling=args.reweight_scaling,
            short_answer_length=args.short_answer_length
            )
    else:
