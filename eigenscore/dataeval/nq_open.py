...
def get_dataset(tokenizer, prompt_mode='brief'):
    ...
    def process_instance(example, prompt_mode='brief'):
        ...
        example['prompt'] = sample_to_prompt({k:example[k] for k in ['question']}, prompt_mode=prompt_mode)
        ...
        return example
    data = data.map(lambda _: process_instance(_, prompt_mode=prompt_mode), load_from_cache_file=False)
    data.set_format(
        type="torch",
