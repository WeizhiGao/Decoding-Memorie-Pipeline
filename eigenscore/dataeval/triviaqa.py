...
def process_data_to_model_inputs(batch, tokenizer, prompt_mode='brief'):
    # assert len(batch['answer']) == 1
    # tokenize the inputs and labels
    answers = [answer["value"] for answer in batch["answer"]]
    batch_with_prompt = sample_to_prompt(batch, prompt_mode)
...
@functools.lru_cache()
def get_dataset(tokenizer, split='validation', prompt_mode='brief'):
    ...
    data = data.map(lambda _: process_data_to_model_inputs(_, tokenizer, prompt_mode),
                            batched=True,
