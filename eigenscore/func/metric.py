...
def get_lenghthNormalized_entropy_list(score_list):
    seq_entropy = np.zeros(len(score_list))
    for idx, score in enumerate(score_list):
        for token_ind in range(len(score)):
            conf, _ = torch.max(score[token_ind].softmax(1), dim=1)
            seq_entropy[idx] += np.log(conf.cpu().numpy())
    normalized_entropy = 0
    for ind, entropy in enumerate(seq_entropy):
        normalized_entropy += entropy/len(score)
    normalized_entropy = -1.0* normalized_entropy/len(score_list)
    return normalized_entropy
...
def getEigenIndicatorOutput(generated_texts, SenSimModel):
    ...
    for ind in range(len(generated_texts)):
        embeddings = SenSimModel.encode(generated_texts[ind], show_progress_bar=False)
...
# A version of getEigenIndicator_v0 for hidden_states_list of multiple generations
def getEigenIndicator_v0_list(hidden_states_list):
    alpha = 1e-3
    selected_layer = int(len(hidden_states_list[0][0])/2)
    if len(hidden_states_list)<2:
        return 0, "None"
    if len(hidden_states_list[0]) >= 2:
        last_embeddings = torch.zeros(len(hidden_states_list), hidden_states_list[0][1][-1].shape[2]).to("cuda")
    else: 
        return 0, None
    for ind in range(len(hidden_states_list)):
        last_embeddings[ind,:] = hidden_states_list[ind][-1][selected_layer][0,0,:]
    CovMatrix = torch.cov(last_embeddings).cpu().numpy().astype(float)
    u, s, vT = np.linalg.svd(CovMatrix+alpha*np.eye(CovMatrix.shape[0]))
    eigenIndicator = np.mean(np.log10(s))
    return eigenIndicator, s
